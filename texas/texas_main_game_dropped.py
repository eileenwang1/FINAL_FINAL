...
class Players():
    ...
    def bet(self, other, pot, your_size = 0, other_size = 0, other_all_in = False):  ### not for fold###
        ...
        if other_all_in:
            ...
            if to_call > self.chips:
                to_call = self.chips
            self.status = '{} goes all-in. Do you want to call or fold?'.format(other.name)
        ...
        if other_size - your_size > 0:
            ...
            self.status = 'Do you want to call, raise, or fold?'
            ...
            if all_in < to_call:
                self.status = 'Do you want to call (all-in), or fold?'
            ...
            all_in = self.chips
            self.status = 'Do you want to check or bet?'
    ...
    def get_bet_size(self):
        message = '{} {}'.format(self.status, self.instructions)
...
def game_start(name_1, name_2):
    pot = Pot()
    player1 = Players(name_1)
    player2 = Players(name_2)
    game(player1, player2, pot)

game_start('Alice', 'Bob')